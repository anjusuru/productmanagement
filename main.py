...
def create_product(product: schemas.Product, db: Session = Depends(get_db)):
    product = crud.create_product(db=db, name=product.name, brand=product.brand, price=product.price,
...
def get_product(id: int, db: Session = Depends(get_db)):
    ...
    if product:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail={"error": f"Product with id {id} does not exist"})
...
def update_product(id: int, product: schemas.Product, db: Session = Depends(get_db)):
    ...
    if db_product:
        updated_product = crud.update_product(db=db, id=id, name=product.name, brand=product.brand, price=product.price,
                                              quantity=product.quantity)
